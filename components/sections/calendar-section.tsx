...
export function CalendarSection() {
  ...
  const [events, setEvents] = useState<{ date: number; type: "festival" | "pooja" | "special" | "event"; label: string }[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedDate, setSelectedDate] = useState<Date | null>(null)

  useEffect(() => {
    fetch("/api/festivals?isActive=true")
      .then((r) => r.json())
      .then((d) => {
          ...
          <div className="space-y-4">
            <Card variant="premium" className="p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-sm font-heading font-bold text-dark-slate">{t("calendar.selectedDate")}</h3>
              ...
              {selectedDate && (
                <div className="p-3 rounded-xl bg-gold-50/50 border border-gold-200/30">
                  <p className="text-xs text-gold-700 font-medium">
                    ...
                    {t("calendar.specialPoojas")}
                  </p>
                </div>
              )}
            </Card>
            <div className="space-y-2">
              <p className="text-xs font-semibold text-text-muted uppercase tracking-wider">{t("calendar.legend")}</p>
              <div className="space-y-1.5">
                {[
                  { color: "bg-maroon-100 border-maroon-200", label: t("calendar.festival") },
                  { color: "bg-gold-100 border-gold-200", label: t("calendar.event") },
                  { color: "bg-emerald-100 border-emerald-200", label: t("calendar.pooja") },
                  { color: "bg-purple-100 border-purple-200", label: t("calendar.special") },
                ].map((item) => (
