...
export async function findDevoteeDetailByPhone(phone: string): Promise<MongoDoc | null> {
  const detail = await db.collection(COLLECTION).findOne({ phone, ...softDeleteFilter() })
...
export async function findDevoteeDetailById(id: string) {
  const detail = await db.collection(COLLECTION).findOne({ _id: toObjectId(id), ...softDeleteFilter() })
