...
export async function findAnnouncementById(id: string): Promise<MongoDoc | null> {
  const announcement = await db.collection(COLLECTION).findOne({ _id: toObjectId(id), ...softDeleteFilter() })
...
export async function findAnnouncementByTitle(title: string): Promise<MongoDoc | null> {
  const doc = await db.collection(COLLECTION).findOne({ title, ...softDeleteFilter() })
