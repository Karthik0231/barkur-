...
const COLLECTION = "donationCampaigns"

export async function findDonationCampaignById(id: string): Promise<MongoDoc | null> {
...
export async function findDonationCampaignBySlug(slug: string): Promise<MongoDoc | null> {
  const doc = await (await getDb()).collection(COLLECTION).findOne({ slug, ...softDeleteFilter() })
  return doc ? { ...doc, id: objectIdToString(doc._id) } as MongoDoc : null
}

export async function findManyDonationCampaigns(filter: Record<string, unknown> = {}, options: { skip?: number; limit?: number; sortBy?: string; sortOrder?: string } = {}): Promise<MongoDoc[]> {
...
export async function countDonationCampaigns(filter: Record<string, unknown> = {}) {
  return (await getDb()).collection(COLLECTION).countDocuments({ ...softDeleteFilter(), ...filter })
}

export async function createDonationCampaign(data: Record<string, unknown>): Promise<MongoDoc> {
...
export async function updateDonationCampaign(id: string, data: Record<string, unknown>): Promise<MongoDoc | null> {
  await (await getDb()).collection(COLLECTION).updateOne(
    { _id: toObjectId(id) },
    { $set: { ...data, updatedAt: new Date() } }
  )
  return findDonationCampaignById(id)
}
