...
export async function findSeoById(id: string) {
...
export async function findSeoByPage(page: string) {
  const doc = await (await getDb()).collection(COLLECTION).findOne({ page })
  return doc ? { ...doc, id: objectIdToString(doc._id) } : null
}

export async function findManySeo(filter: Record<string, unknown> = {}, options: { skip?: number; limit?: number; sortBy?: string; sortOrder?: string } = {}) {
  const { skip, limit, sortBy = "page", sortOrder = "asc" } = options
